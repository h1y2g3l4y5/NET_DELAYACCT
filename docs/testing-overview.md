...
而 Δ% 随包变大被 1/尺寸 摊薄（64B 格 ~16% → 65000B 格 <1%，低于噪声
地板判 INVALID 是预期行为，不是失败）。这张矩阵就是"包大影响小"论断
的实证数据。
...
## 5. 典型结果解读（run #182，报告 20260818_011516，矩阵化前单点口径）
...
> v2 矩阵化后（24 格）预期形态：64B/1400B 格 PASS（Δns ~500-800 且
> Δ% > 地板），65000B 格 INVALID（Δns 平稳但 Δ% <1% 落入地板下），
> 两者共同构成"按包收税、包大摊薄"的完整规律曲线。
...
- 每格 3 轮 bench（每轮自动校准 ~1s）；K0/K3 各 2 次开机共 6 样本取中位数
