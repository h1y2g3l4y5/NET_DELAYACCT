...
if [ -d "$TRACEFS" ] && [ -w "$TRACEFS/kprobe_events" ]; then
	...
	if grep -q 'tx_start' "$TRACEFS/kprobe_events" 2>/dev/null && \
	   grep -q 'tx_end' "$TRACEFS/kprobe_events" 2>/dev/null; then
		...
		# - 辅助断言：计数比 ∈ [50%, 300%]（容忍纯 ACK 守卫 + GSO 分段 + 共享 runner 调度噪声）
	#   200% → 250% → 300%：CI run #141 ratio=209% / #142 ratio=203% 在共享 runner 上偶发超 200%
	#   P6 改动（tx_end 从 dev_hard_start_xmit 移到 __dev_queue_xmit）后 GSO 不再膨胀
	#   tx_end 计数，但 __dev_queue_xmit 覆盖更广路径（含 qdisc 前所有报文），
	#   CI run #155 ratio=251% 刚超 250%，提升到 300% 给 ~20% 余量。
	#   纯 ACK / 窗口更新 skb 经过 tx_end 但不经过 tx_start，ACK 数量受调度噪声影响偶尔超 2x
	#   > 300% 仍判定 FAIL（捕获真正的多打点 bug）
		...
		if [ "$TX_START_COUNT" -gt 0 ] && [ "$TX_END_COUNT" -gt 0 ]; then
			...
			if [ "$MISMATCHED_N" -le "$MISMATCH_THRESHOLD" ] && [ "$RATIO" -ge 50 ] && [ "$RATIO" -le 300 ]; then
				_pass "per-skb pairing OK: mismatched=$MISMATCHED_N/$TX_END_UNIQUE (threshold=$MISMATCH_THRESHOLD, ACK-tolerant), start_unique=$TX_START_UNIQUE, ratio=${RATIO}% (within [50%, 300%])"
			else
				...
				if [ "$RATIO" -lt 50 ] || [ "$RATIO" -gt 300 ]; then
					...
					echo "    | tx_end/tx_start = ${RATIO}% (expect [50%, 300%])"
					echo "    +---------------"
				fi
				_fail "per-skb pairing or ratio check failed: mismatched=$MISMATCHED_N (threshold=$MISMATCH_THRESHOLD) ratio=${RATIO}% (expect mismatched<=threshold, ratio in [50%,300%])"
			fi
		else
