...
- **报告生成日期**: 2026-07-28
- **覆盖 Review 版本**: v2.0.0, v3.0.0, v4.0.0, v5.0.0
- **项目状态**: 已完成 5 轮完整 Review，全部闭环，零遗留问题
- **最终项目评分**: 9.5/10 (v5.0.0)
...
| v5.0.0 | API 演进与功能扩展 | ✅ 闭环 | 9.5/10 | 2026-07-28 | Netlink dump 化、用户态过滤、UAPI 注释 |

**演进趋势**:
- v2.0.0 是"打地基"：从 6.2 分逐步提升到 7.0 分，解决基础工程正确性
- v3.0.0 是"修核心"：从 7.0 分提升到 9.5 分，解决打点准确性和路径覆盖
- v4.0.0 是"做增强"：稳定在 8.5 分，补充 min/max、溢出检测、文档化
- v5.0.0 是"扩能力"：从 8.5 分提升到 9.5 分，完成 Netlink dump 化和用户态过滤
...
### 2.4 v5.0.0 API 演进与功能扩展

**6 项议题全部闭环**：

| 编号 | 优先级 | 问题 | 最终状态 | 关键修复 |
|------|--------|------|----------|----------|
| ISSUE-3 | P1 | Netlink 非标准 dump 协议 | ✅ 已闭环 | GET_BY_PID 改为 `.start/.dumpit/.done` + `cb->ctx` |
| REV-1 | P2 | `.start` 隐式依赖框架 zero-init | ✅ 已闭环 | 入口显式 `memset(cb->ctx, 0)` |
| REV-2 | P3 | `.dumpit` 末尾返回 `skb->len` | ✅ 已闭环 | 改为 `return 0` |
| ISSUE-5 | P1 | 用户态缺少过滤功能 | ✅ 已闭环 | 内核 `net_delayacct_match_filter()` + 用户态 6 维过滤 CLI |
| ISSUE-6 | P2 | UAPI 属性扩展需保证兼容性 | ✅ 已闭环 | 属性角色注释 `[KEY]`/`[REQ filter]`/`[REPLY]` |
| ISSUE-7 | P2 | 缺少过滤功能测试 | ✅ 已闭环 | 新增 Test 14-16，修复 Test 16 baseline 干扰 |

**关键隐藏 bug 修复**: `fill_sock()` 与 `net_delayacct_match_filter()` 中对 `sk->sk_num` 错误使用 `ntohs()`，导致本地端口显示与过滤字节翻转（如 21416 显示为 43091）。修复后 `lport = sk->sk_num`（host order），`rport = ntohs(sk->sk_dport)`（network order）。
...
8. **Netlink dumpit 重构** (v5.0.0)
   - `GET_BY_PID` 改为标准 `.start/.dumpit/.done`
   - 内核 6.6 中 `cb->ctx` 是 `u8 ctx[48]` 内联数组，直接 cast 40 字节结构体
   - `GET_BY_INODE` 保持 doit，保留 `net_delayacct_one_reply()`

9. **用户态过滤协议** (v5.0.0)
   - 复用现有 UAPI 属性作为过滤输入，遵循 inet_diag 约定，零 ABI 风险
   - 6 维可选过滤：proto/family/lport/rport/laddr/raddr
   - 地址比较按 socket family 区分 IPv4/IPv6

### 3.2 修改文件清单（按轮次聚合）

**内核源码**:
- `include/uapi/linux/net-delayacct.h` — UAPI stats 结构、命令 ID、属性角色、RESET 语义
- `include/net/net-delayacct.h` — 内部结构、inline helpers、min/max 初始化、RESET kerneldoc
- `net/core/net-delayacct.c` — 核心模块、命令处理、dumpit、过滤、min/max 更新、溢出检测
...
**用户态工具**:
- `userspace/get_sockdelays/get_sockdelays.c` — CLI、JSON、min/max 显示、6 维过滤
- `userspace/get_sockdelays/Makefile` — `-I.` 支持本地 UAPI 回退
- `userspace/get_sockdelays/linux/net-delayacct.h` — 用户态 UAPI 回退头

**CI/QEMU**:
- `ci/qemu/run-tests.sh` — 16 项测试套件（含过滤测试）
...
- `kernel-patches/include-net-net-delayacct.h`
...
| v5.0.0 ISSUE-3 | dumpit 重构，13/13 PASS |
| v5.0.1 | REV-1/REV-2 防御性改进，13/13 PASS |
| v5.0.2/5.0.3 | 过滤功能 + 4 个健壮性问题修复，16/16 PASS，评分 9.5/10 |
...
- min/max 统计正确性
- `--proto`/`--lport`/`--family` 过滤功能
- `--proto tcp --lport` 组合过滤 AND 语义
...
| v5.0.0 API 演进审查 | `logs/review/v5.0.0/REVIEW_REPORT_v5.0.0_api-evolution.md` |
| v5.0.1 dumpit 修复验证 | `logs/review/v5.0.0/REVIEW_REPORT_v5.0.1_dumpit-fix-validation.md` |
| v5.0.2 过滤功能验证 | `logs/review/v5.0.0/REVIEW_REPORT_v5.0.2_filtering-validation.md` |
| v5.0.3 修复验证 | `logs/review/v5.0.0/REVIEW_REPORT_v5.0.3_fix-validation.md` |
| v5.0.0 最终总结 | `logs/summary/v5.0.0_FINAL_REPORT.md` |
| 跨轮次总结（本文件） | `logs/summary/PROJECT_REVIEW_SUMMARY_v2-v5.md` |
| 对话记录 | `logs/dialogue/DLG-20260726-134700.md` 等 |
| 项目记忆 | `/home/lai/.trae-cn/memory/projects/-home-lai-Code-NET-DELAYACCT/project_memory.md` |

### 5.2 工作日志资产

- `logs/work/2026-07-27/`: TASK-06 ~ TASK-17（v2.0.0 复审、v3.0.0 三轮修复）
- `logs/work/2026-07-28/`: TASK-18 ~ TASK-25（v4.0.0 修复、v5.0.0 dumpit/过滤/闭环）
...
5. **边界条件必须枚举**: MSG_PEEK/corked/splice/zerocopy/retransmit 等不能依赖主路径测试，必须逐一静态分析。
6. **内核字段端序必须核对类型定义**: `sk->sk_num`（`__u16`, host order）与 `sk->sk_dport`（`__be16`, network order）端序不同，不能统一套用 `ntohs()`。
...
5. **patch 作者身份和版权必须统一**: UAPI 头文件早期占位版权 `h1y2g3l4y5` 与 `Signed-off-by` 不一致，upstream 提交前应统一为 `laiguo-liang`。

### 6.3 设计文档层面

1. **头文件注释是 API 契约的一部分**: min/max 初始化语义、RESET 非全局快照、GRO/GSO 粒度、过滤属性角色等设计权衡必须在 UAPI/kerneldoc 中清晰记录。
2. **统计语义要文档化**: 用户容易误解 RESET 为"全局原子清零"，必须显式说明其遍历语义。
3. **commit message 要记录设计权衡**: 如 TCP TX clone-block 时间戳、UDP rx_end post-checksum 位置、dumpit 重构原因等，便于后续维护者理解。

### 6.4 测试设计层面

1. **测试断言应基于输出格式而非环境行为**: v5.0.2 的端口正则问题说明断言不能依赖当前环境（IPv6 dual-stack）的绑定行为。
2. **第三方工具行为需要充分了解**: iperf3 server 单线程处理同一端口多个 client，UDP 数据 socket 生命周期与 client 绑定，设计测试时必须纳入考虑。
3. **使用非对称值验证端序**: 端口测试应使用 21416 等非对称值，而非 80/443 等字节翻转后仍可能"正确"的值。
...
基于 v5.0.0 遗留的关注点和长期设计增强需求，建议 v6.0.0 重点处理：

| 优先级 | 议题 | 工作要点 |
|--------|------|----------|
| P2 | 过滤属性策略校验 | 确认内核对 `LADDR/RADDR` 的 `NLA_POLICY_MIN_LEN` 执行校验，避免畸形属性进入过滤函数 |
| P2 | IPv4-mapped IPv6 过滤语义 | 在 UAPI 文档中显式说明"过滤地址的 family 必须与 socket 实际 family 一致" |
| P2 | 端口范围校验 | `--lport`/`--rport` 当前对非数字输入静默转为 0，建议增加输入校验 |
| P2 | 大 socket 集过滤性能 | 当前每个 socket 都通过 `genl_info_dump(cb)` 解引用 info；若需支持数千 socket 高频查询，可考虑缓存过滤条件 |
| P2 | 标准差/分位数统计 | 在 stats 结构中增加方差累积量，支持 P95/P99/P999 |
| P3 | HDR Histogram 直方图 | 按对数桶分布延迟，便于 tail latency 分析 |
...
NET_DELAYACCT 项目经过 v2.0.0/v3.0.0/v4.0.0/v5.0.0 四轮完整 Review，已完成从"概念验证"到"设计完整、路径覆盖全面、API 标准、功能丰富、文档清晰"的演进：
...
- **API 演进**: 已将 `GET_BY_PID` 改为标准 Generic Netlink dump 协议
- **功能扩展**: 已实现 6 维用户态过滤（proto/family/lport/rport/laddr/raddr）
- **验证体系**: QEMU 16/16 PASS，并发压力测试通过，checkpatch 0 errors

**当前项目状态**: ✅ 四轮 Review 全部闭环，零遗留问题，具备进入 v6.0.0 长期增强阶段的基础。
