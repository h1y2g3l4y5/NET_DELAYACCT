...
write_summary_files() {
    ...
    # Markdown
    {
        ...
        echo ""
        echo "- **时间戳**: ${TIMESTAMP}"
        echo "- **ON 内核模式**: ${on_mode:-unknown}"
        echo "- **OFF 内核模式**: ${off_mode:-unknown}"
        echo ""
        echo "## 指标详情"
        echo ""
        echo "| 指标 | 单位 | ON 原始值 | OFF 原始值 | ON 中位数 | OFF 中位数 | 差值(绝对) | 差值(百分比) | 阈值 | 判定 |"
        echo "|------|------|-----------|------------|-----------|------------|-------------|-------------|------|------|"
        while IFS=$'\t' read -r metric unit on_raw off_raw on_med off_med delta_abs delta_pct threshold verdict; do
            [ -z "$metric" ] && continue
        ...
        echo "## Delta 计算方向"
        echo ""
        echo "- TCP 吞吐量: (OFF - ON) / OFF * 100（正值 = 吞吐下降）"
        echo "- UDP PPS: (OFF - ON) / OFF * 100（正值 = PPS 下降）"
        echo "- TCP 延迟: (ON - OFF) / OFF * 100（正值 = 延迟增加）"
        echo "- CPU 利用率: (ON - OFF) / OFF * 100（正值 = CPU 增加）"
        echo "- Socket 对象大小: ON - OFF（字节）"
    } > "$SUMMARY_MD"
